...
TEST_CASE(side_orders)
{
    ...
    auto orange = create_order(pool, 2, *trader, *accnt, *contr, 20130827,
                               "orange", DBR_BUY, 12345, 20, 0, 0, now);
    ...
    check(apple->resd == 10);
    check(apple->exec == 0);
    check(apple->last_ticks == -1);
    check(apple->last_lots == -1);
    ...
    check(orange->resd == 0);
    check(orange->exec == 0);
    check(orange->last_ticks == -1);
    check(orange->last_lots == -1);
