              ...
              const char* unix_socket, unsigned long client_flag)
{
    ...
    if (!mysql_real_connect(db.get(), host, user, pass, dbname, 0, unix_socket, client_flag)) {
        throw DatabaseException{err_msg() << "mysql_real_connect: " << mysql_error(db.get())};
    }
    return db;
}
...
void query(MYSQL& db, std::string_view sql)
{
    if (mysql_real_query(&db, sql.data(), sql.size()) != 0) {
        throw DatabaseException{err_msg() << "mysql_real_query: " << mysql_error(&db)};
    }
}
...
ResPtr store_result(MYSQL& db)
{
    ...
    if (!res) {
        throw DatabaseException{err_msg() << "mysql_store_result: " << mysql_error(&db)};
    }
    return res;
}
...
StmtPtr prepare(MYSQL& db, std::string_view sql)
{
    ...
    if (!stmt) {
        throw DatabaseException{err_msg() << "mysql_stmt_init: " << mysql_error(&db)};
    }
    if (mysql_stmt_prepare(stmt.get(), sql.data(), sql.size()) != 0) {
        throw DatabaseException{err_msg()
                                << "mysql_stmt_prepare: " << mysql_stmt_error(stmt.get())};
    }
    return stmt;
}
...
void execute(MYSQL_STMT& stmt)
{
    if (mysql_stmt_execute(&stmt) != 0) {
        throw DatabaseException{err_msg() << "mysql_stmt_execute: " << mysql_stmt_error(&stmt)};
    }
}
ResPtr result_metadata(MYSQL_STMT& stmt)
{
    ...
    if (!res) {
        throw DatabaseException{err_msg()
                                << "mysql_stmt_result_metadata: " << mysql_stmt_error(&stmt)};
    }
    return res;
}
...
void bind_param(MYSQL_STMT& stmt, MYSQL_BIND* bind)
{
    if (mysql_stmt_bind_param(&stmt, bind) != 0) {
        throw DatabaseException{err_msg() << "mysql_stmt_bind_param: " << mysql_stmt_error(&stmt)};
    }
}
...
void bind_result(MYSQL_STMT& stmt, MYSQL_BIND* bind)
{
    if (mysql_stmt_bind_result(&stmt, bind) != 0) {
        throw DatabaseException{err_msg() << "mysql_stmt_bind_result: " << mysql_stmt_error(&stmt)};
    }
}
...
bool fetch(MYSQL_STMT& stmt)
{
    ...
    case MYSQL_DATA_TRUNCATED:
        throw DatabaseException{err_msg() << "mysql_stmt_fetch: data truncated"};
    case 1:
    default:
        throw DatabaseException{err_msg() << "mysql_stmt_fetch: " << mysql_stmt_error(&stmt)};
    }
    return true;
}
