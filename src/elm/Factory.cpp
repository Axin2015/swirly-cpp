...
{
    return doNewOrder(trader, market, contr, settlDay, id, ref, state, side, lots, ticks,
                      resd, exec, cost, lastLots, lastTicks, minLots, created, modified);
}

OrderPtr Factory::newOrder(const StringView& trader, const StringView& market,
                           const StringView& contr, Jday settlDay, Iden id,
                           const StringView& ref, Side side, Lots lots, Ticks ticks,
                           Lots minLots, Millis created) const
{
    return doNewOrder(trader, market, contr, settlDay, id, ref, State::NEW, side, lots, ticks,
                      lots, 0_lts, 0_cst, 0_lts, 0_tks, minLots, created, created);
}
...
{
    return doNewExec(trader, market, contr, settlDay, id, ref, orderId, state, side, lots,
...
ExecPtr Factory::newExec(const Order& order, Iden id, Millis created) const
{
    return doNewExec(order.trader(), order.market(), order.contr(), order.settlDay(), id,
                     ...
                     order.ticks(), order.resd(), order.exec(), order.cost(),
                     order.lastLots(), order.lastTicks(), order.minLots(), 0_id,
                     Role::NONE, StringView{}, created);
}
...
{
    return makeRefCounted<Order>(trader, market, contr, settlDay, id, ref, state, side, lots,
                                 ticks, resd, exec, cost, lastLots, lastTicks, minLots,
                                 created, modified);
}
