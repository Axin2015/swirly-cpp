...
Product::Impl::Impl(const QString& symbol, const QString& display, const QString& base_asset,
...
Product Product::from_json(const QJsonObject& obj)
{
    using swirly::ui::from_json;
    return Product(from_json<QString>(obj["symbol"]), from_json<QString>(obj["display"]),
...
shared_ptr<const Product::Impl> Product::empty()
{
...
{
    debug.nospace() << "Product{symbol=" << product.symbol()  //
                    << ",display=" << product.display()       //
                    << ",base_asset=" << product.base_asset() //
                    << ",term_ccy=" << product.term_ccy()     //
                    << ",lot_numer=" << product.lot_numer()   //
                    << ",lot_denom=" << product.lot_denom()   //
                    << ",tick_numer=" << product.tick_numer() //
                    << ",tick_denom=" << product.tick_denom() //
                    << ",pip_dp=" << product.pip_dp()         //
                    << ",min_lots=" << product.min_lots()     //
                    << ",max_lots=" << product.max_lots()     //
                    << '}';
    return debug;
}
