...
#include <scpp/elm/perm.hpp>
#include <scpp/elm/posn.hpp>
#include <scpp/elm/view.hpp>

#include <scpp/ash/mutex.hpp>
#include <scpp/ash/shlex.hpp>
...
AccntRecRef
get_arec(ClntRef clnt, ScIden id)
{
...
ContrRecRef
get_crec(ClntRef clnt, ScIden id)
{
...
public:
    ...
    on_async(ClntRef clnt, void* val) noexcept
    {
        try {
            ...
            sc_log_error("exception: {}", e.what());
        } catch (const InvalidArgument& e) {
            sc_log_error("invalid argument: {}", e.what());
        } catch (const InvalidState& e) {
            sc_log_error("invalid state: {}", e.what());
        }
        return nullptr;
    }
    // This.
        ...
        call(async, [umnem, &width, &rows](ClntRef clnt) {
                ...
                for (auto order : Accnt(clnt.accnt(trec)).orders()) {
                    ...
                    row r{
                        ...
                        to_string(sc_jd_to_iso(ref.settl_day())),
                        state_str(ref.state()),
    ...
    depth(Async& async, Arg begin, Arg end)
    {
        ...
        const auto settl_day = sc_iso_to_jd(ston<ScIsoDate>(get("settl_date").c_str()));
    ...
    buy(Async& async, Arg begin, Arg end)
    {
        ...
        const string cmnem = get("contr");
        const auto settl_day = sc_iso_to_jd(ston<ScIsoDate>(get("settl_date").c_str()));
        const auto lots = ston<ScLots>((*begin++).c_str());
        ...
        call(async, [umnem, gmnem, cmnem, settl_day, price, lots](ClntRef clnt) {
                ...
                clnt.place(clnt.accnt(trec), clnt.accnt(grec), crec, settl_day,
                           nullptr, SC_ACTION_BUY, crec.price_to_ticks(price), lots, 0);
            });
    }
    void
    sell(Async& async, Arg begin, Arg end)
    {
        ...
        call(async, [umnem, gmnem, cmnem, settl_day, price, lots](ClntRef clnt) {
                ...
                auto crec = get_crec(clnt, cmnem.c_str());
                clnt.place(clnt.accnt(trec), clnt.accnt(grec), crec, settl_day,
                           nullptr, SC_ACTION_SELL, crec.price_to_ticks(price), lots, 0);
            });
    }
    void
    ...
    {
        const string umnem = get("trader");
        const auto id = ston<ScIden>((*begin++).c_str());
        const auto lots = ston<ScLots>((*begin++).c_str());
...
class LogBuffer {
    typedef tuple<ScMillis, int, string> Entry;
    Mutex mutex_;
    list<Entry> entries_;
    void
...
public:
    ...
    {
        Lock l(mutex_);
        entries_.emplace_back(sc_millis(), level, msg);
    }
    void
    flush()
    {
...
main(int argc, char* argv[])
{
    ...
    try {
        ...
        char path[PATH_MAX];
        sprintf(path, "%s/.scshrc", getenv("HOME"));
        ifstream cfg(path);
        if (cfg.is_open())
            repl.read(cfg);
        repl.read(cin, "scsh> ");
