 ...
 */
#ifndef SC_ASH_DATE_H
#define SC_ASH_DATE_H
...
static inline ScIsoDate
sc_ymd_to_iso(int year, int mon, int mday)
{
    assert(mon <= 12);
    assert(mday <= 31);
...
static inline struct ScYmd*
sc_iso_to_ymd(ScIsoDate iso, struct ScYmd* ymd)
{
    ymd->year = iso / 10000;
    ymd->mon = iso / 100 % 100;
    ymd->mday = iso % 100;
...
static inline ScJd
sc_ymd_to_jd(int year, int mon, int mday)
{
...
static inline struct ScYmd*
sc_jd_to_ymd(ScJd jd, struct ScYmd* ymd)
{
    // The formula given above was taken from the 1990 edition of the U.S. Naval Observatory's
    // Almanac for Computers.
    // See http://aa.usno.navy.mil/faq/docs/JD_Formula.php.
...
static inline ScJd
sc_iso_to_jd(ScIsoDate iso)
{
    struct ScYmd ymd;
    sc_iso_to_ymd(iso, &ymd);
    return sc_ymd_to_jd(ymd.year, ymd.mon, ymd.mday);
}
...
static inline ScIsoDate
sc_jd_to_iso(ScJd jd)
{
    struct ScYmd ymd;
    sc_jd_to_ymd(jd, &ymd);
    return sc_ymd_to_iso(ymd.year, ymd.mon, ymd.mday);
}
